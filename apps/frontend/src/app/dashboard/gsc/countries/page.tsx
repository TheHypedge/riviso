...
function CountriesPageContent() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data } = await api.get<CountryData[]>('/v1/search-console/countries', {
        params: {
      ...
      setError(e?.response?.data?.message || 'Failed to fetch countries data');
      setCountries([]);
    } finally {
      setLoading(false);
    }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Analyze search performance by country</p>
        </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {countries.map((country, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500 text-right">{country.clicks.toLocaleString()}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 text-right">{country.impressions.toLocaleString()}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 text-right">{country.ctr.toFixed(2)}%</td>
                      <td className="px-6 py-4 text-sm text-gray-500 text-right">{country.position.toFixed(1)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {countries.length === 0 && !loading && !error && (
          <div className="bg-white rounded-xl shadow-md p-8 border border-gray-100 text-center">
            ...
            <p className="text-gray-600 font-medium">No country data available</p>
            <p className="text-sm text-gray-500 mt-2">Click "Refresh Data" to fetch country performance data</p>
          </div>
        )}
      </div>
    </DashboardLayout>
